...
/**
    ...
    \note   RCMessageContent 已经实现此协议方法，派生类需按照自己需求重新覆盖实现，
 */
@protocol RCMessagePersistentCompatible <NSObject>
@required
/** 
    \brief  返回遵循此protocol的类对象持久化的标识
    \return 返回持久化设定标识
    \note   默认实现返回 \const (MessagePersistent_ISPERSISTED | MessagePersistent_ISCOUNTED)
*/
...
/**
    \brief 消息体基类
    \note 所有自定义消息需继承此类，并当实现其遵循的两个协议接口
 */
